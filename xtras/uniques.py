allplaces = ["45 Tchoup (20 New Orleans Dive Bars to Avoid Valentine’s Day )",
...
#Filters
remove_parenthesis = compile(r"\s\(.*\)$")
remove_event = compile(r"['’]s.*|^.*\sat\s|^.*\sin the\s") #everything after apostrophe, before "in the", at
remove_venue_type = compile(r"(?<!^)\s" + r"|(?<!^)\s".join(remove_from_place)) 
# remove_event2 = compile(r"")
# remove_event3 = compile(r"^.*\sin the\s") #everython before at

justthename = compile(r"(?<!^)\s" + r'|(?<!^)\s'.join(remove_from_place) +r"|\s\(.*\)|^.*\sat\s|[pP]o-?[bB]oys?|['’]s.*")
nameandkind = compile(r"\s\(.*\)|^.*\sat\s")

#print(remove_venue_type.pattern)
...
for place in allplaces:
	...
	nplace = normalize('NFKC', place)


	nplace = remove_parenthesis.sub("", nplace)
	nplace = remove_event.sub("", nplace)

	fullname = nplace

	nplace = remove_venue_type.sub("", nplace)
	...
	restaurants[name] = {'fullname' : fullname}



for idx, (slug,rest) in enumerate(restaurants.items()):
	#print("{}.{}  :  {}".format(idx,slug,rest['fullname']))
	print("{}  :  {}".format(slug,rest['fullname']))
